    ...
    <tabset class="tab-animation">
        <tab heading="管理儲存庫">
            <div class="new-orders" collapse="!(data.orderList | filter:isCheck).length">
                ...
                <table class="table table-striped">
                    ...
                    <tr ng-repeat="order in data.orderList | filter:isCheck">
                        ...
                        <td>
                            <button class="btn btn-primary" ng-click="checkout(order)" ng-disabled="data.loading">核可
                            </button>
                            <button class="btn btn-danger" ng-click="reject(order)" ng-disabled="data.loading">拒絕
                            </button>
                        </td>
                    </tr>
                    </tbody>
            ...
            <div class="item-list">
                ...
                <div class="row form-group">
                    <div class="col-sm-10"><input type="text" class="form-control" placeholder="搜尋"
                                                  ng-model="input.itemName" ng-disabled="stage.show"></div>
                    <div class="col-sm-2">
                        <button class="btn btn-primary" ng-click="input.itemName = null" ng-disabled="stage.show">
                            Clear
                        </button>
                    </div>
                </div>
                <div class="table-responsive">
                    <table id="item-list" class="table item-list table-striped">
                        <thead>
                        <th>料號</th>
                        <th>物品名稱</th>
                        <th>規格</th>
                        ...
                        <tbody>
                        <tr ng-repeat="item in data.itemList | orderBy:percent | filter:input.itemName"
                            ng-class="{'danger': item.count < item.low_floor, 'warning': item.count/item.low_floor < 1.2}">
                            <td>{{item.list_key}}</td>
                            <td>{{item.name}}</td>
                            <td>{{item.spec}}</td>
                            <td>
                                <span ng-click="showSupplier(item.supplier)"
                                      ng-class="{'btn-link': inSuppliers(item.supplier)}">{{item.supplier}}</span>
                            </td>
                            ...
                            <td class="text-right">{{((item.count - item.low_floor)/item.low_floor*100).toFixed(0)}}%
                            </td>
                            <td class="text-right">{{item.comment}}</td>
                            <td>
                                <button class="btn btn-danger" ng-click="addCount(item)">進貨</button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
        ...
        <tab heading="耗材清單">
            ...
            <div class="form-group">
                ...
                <input type="text" placeholder="輸入關鍵字" id="search-item" class="form-control"
                       ng-model="input.searchItem">
            </div>
        ...
        <tab heading="廠商清單">
            ...
            <div class="form-group">
                ...
                <input type="text" placeholder="輸入關鍵字" id="search-supplier" class="form-control"
                       ng-model="input.searchSupplier">
            </div>
            <div class="table-responsive">
                <table class="table">
                    <thead>
            ...
            <tr>
                ...
                <td>
						<span class="text-success"
                              ng-class="{'text-warning': param.item.count/param.item.low_floor < 1.2, 'text-danger': param.item.count <= param.item.low_floor}">
							{{param.item.count}}
						</span>
                    &rarr;
                    <span class="text-success"
                          ng-class="{'text-warning': (param.item.count + param.number)/param.item.low_floor < 1.2, 'text-danger': (param.item.count + param.number) <= param.item.low_floor}">
...
<script type="text/ng-template" id="edit-item.html">
    <div class="row">
        ...
        <div class="form-group col-sm-6">
            ...
            <input type="number" class="form-control form-lg" ng-model="param.price" id="price" step="0.01"
                   placeholder="輸入價格">
        </div>
        <div class="form-group col-sm-6">
            <label for="dimension">單位</label>
            <input type="text" class="form-control form-lg" ng-model="param.dimension" id="dimension"
                   placeholder="輸入單位">
        </div>
        <div class="form-group col-sm-6">
        ...
        <div class="form-group col-md-12">
            ...
            <textarea ng-model="param.comment" class="form-control form-lg" id="comment" rows="4"
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td>
                    ...
                    &rarr;
                    <span class="text-success"
                          ng-class="{'text-warning': (param.item.count - param.order.number)/param.item.low_floor < 1.2, 'text-danger': (param.item.count - param.order.number) <= param.item.low_floor}">
...
<script type="text/ng-template" id="edit-supplier.html">
    <div class="row">
        ...
        <div class="form-group col-sm-6">
            ...
            <input type="text" placeholder="輸入統編" id="reciept_number" class="form-control"
                   ng-model="param.receipt_number">
        </div>
